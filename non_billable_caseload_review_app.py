...
st.markdown(
    ...
    section[data-testid="stSidebar"] {
        background:
            radial-gradient(circle at top, rgba(14,165,233,.22), transparent 24%),
    ...
    .sidebar-brand {
        padding: 1rem .75rem 1.15rem .75rem;
        margin-bottom: .75rem;
    ...
    .sidebar-logo {
        ...
        height: 54px;
        border-radius: 20px;
        background: linear-gradient(135deg, rgba(6,182,212,.9), rgba(37,99,235,.72));
        display: flex;
        align-items: center;
        justify-content: center;
        font-size: 1.55rem;
        box-shadow: 0 0 28px rgba(14,165,233,.22);
    ...
    .nav-pill.active {
        ...
        box-shadow: inset 3px 0 0 rgba(56,189,248,.95), 0 14px 30px rgba(2, 132, 199, .08);
        color: #ffffff;
    }
    ...
    .sidebar-mini-title {
        ...
        margin: 1.25rem .75rem .4rem .75rem;
    }
    ...
    .metric-card {
        position: relative;
        overflow: hidden;
        background: linear-gradient(145deg, rgba(15,23,42,.92), rgba(30,41,59,.64));
    ...
    .review-button-card {
        ...
        background: linear-gradient(145deg, rgba(15,23,42,.92), rgba(30,41,59,.54));
...
st.sidebar.markdown(
    ...
    <div class="sidebar-brand">
        ...
        <div class="sidebar-title">Non-Billable Caseload Review Dashboard v2</div>
        <div class="sidebar-subtitle">Employee-specific supervisory accountability dashboard</div>
    </div>
    <div class="nav-pill active">🏠 <span>Dashboard</span></div>
    <div class="nav-pill">📈 <span>Executive Summary</span></div>
    <div class="nav-pill">✅ <span>Successful Engagement</span></div>
    <div class="nav-pill">📞 <span>Attempt Activity</span></div>
    <div class="nav-pill">📄 <span>Non-Billable Activity</span></div>
    <div class="nav-pill">🚩 <span>No Engagement / No Attempts</span></div>
    <div class="sidebar-mini-title">Downloads</div>
    <div class="nav-pill">⬇️ <span>Selected List</span></div>
    <div class="nav-pill">🖨️ <span>Print View</span></div>
    <div class="nav-pill">📚 <span>Audit Workbook</span></div>
    """,
    unsafe_allow_html=True,
)
...
cols = st.columns(5)
with cols[0]:
    metric_card("Total Caseload", f"{total_caseload:,}", "Clients on caseload", "👥", "metric-cyan")
with cols[1]:
    metric_card(
...
cols2 = st.columns(4)
with cols2[0]:
    metric_card("Total Services Rendered", f"{total_services_rendered:,}", "All rows in MyServicesList", "🧾", "metric-cyan")
with cols2[1]:
    metric_card("Billable Service Rows", f"{successful_engagement_rows:,}", "Rows excluding non-billables and attempts", "🟢", "metric-green")
with cols2[2]:
    metric_card("No Show / Cancel Total", f"{no_show_cancel_total:,}", "Rows with attempt status", "⚠️", "metric-amber")
with cols2[3]:
    metric_card("Total Units", f"{total_units:,.0f}", "Units documented in MyServicesList", "🔢", "metric-purple")
...
button_specs = [
    ("Successful Engagements", "✅", "Billable/show activity", "rgba(34,197,94,.15)"),
    ("Non-Billable Totals", "📄", "Non-billable + attempts", "rgba(124,58,237,.15)"),
    ("Attempt Only / No Contact", "📞", "Only attempt/non-success activity", "rgba(245,158,11,.16)"),
    ("No Show / Cancel Total", "⚠️", "Attempt status rows", "rgba(249,115,22,.16)"),
    ("No Engagement / No Attempts", "🚩", "Missing from MyServicesList", "rgba(244,63,94,.16)"),
    ("All Caseload", "👥", "Full caseload review", "rgba(14,165,233,.16)"),
]

for col, (label, icon, sub, bg) in zip(button_cols, button_specs):
    with col:
        st.markdown(
            f"""
            <div class="review-button-card">
